...
Все инструменты, принимающие instrument (place_order, place_stop_order, get_last_price, get_order_book, правила стратегий), SHALL принимать ссылку в одном из форматов: {ticker,class_code}, {figi}, {uid}. Сервер SHALL резолвить ссылку в figi для торговых операций и кэшировать результат; неразрешимая ссылка SHALL возвращать ошибку.
...
Сервер SHALL отдавать все денежные величины (цены, суммы) как десятичные строки (например "340.50") с указанием валюты: value и currency. Никакие величины SHALL NOT округляться молчаливо: дробная часть воспроизводится из Quotation без потерь.
